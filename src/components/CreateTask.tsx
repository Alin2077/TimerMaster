  ...
  return (
    <div className="card">
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
        ...
        ] as { key: RepeatType; label: string }[]).map((r) => (
        ...
        <div style={{ marginBottom: 12 }}>
          <div className="presets">
            {INTERVAL_PRESETS.map((p) => (
              <button key={p.label} className="preset-btn"
                onClick={() => setIntervalMin(String(p.mins))}
        ...
        {CATEGORIES.map((cat) => (
          <button key={cat} onClick={() => setCategory(cat)}
            style={{
              ...
              border: `1px solid ${category === cat ? CAT_COLORS[cat] : "var(--border-color)"}`,
              background: category === cat ? `${CAT_COLORS[cat]}33` : "transparent",
              color: category === cat ? CAT_COLORS[cat] : "var(--text-secondary)",
              fontSize: 12, cursor: "pointer",
            }}>{cat}</button>
        ))}
      </div>
        ...
        {(["none","shutdown","open","script"] as ActionType[]).map((t) => (
          <button key={t} onClick={() => { setActionType(t); setActionPath(""); }}
            style={{
              ...
              background: actionType === t ? "var(--accent-purple)" : "transparent",
      ...
      {actionType === "shutdown" && (
        <div style={{ fontSize: 11, color: "var(--accent-orange)", marginBottom: 12 }}>
          ⚠️ 触发 30 秒后关机，可运行 shutdown /a 取消
        </div>
      )}
